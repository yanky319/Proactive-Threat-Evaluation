...
class CybereasonScraper:
    ...
    def find_new_blogs(self):
        ...
        reports = soup.find_all("div",
                                class_="column is-4-fullhd is-6-desktop is-half-tablet is-full-mobile blog-listing__single-post")
        for report in reports:
            a = report.find(class_='text-content-bundle').find('a')
            link = a.get("href")

            date_string = report.find(class_='text-content-bundle').find_all("p")[-1].find_all("span")[0].get_text()[
                          :-2]
